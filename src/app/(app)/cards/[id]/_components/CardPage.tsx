...
export function CardPage({ card }: { card: any }) {
  const cardArt = card.card_art as Media;
  console.log(getCardColorBG(card.rune))
  return (
    <div key={card.id} className="mx-auto w-full max-w-screen-xl p-4 py-6 lg:py-8 flex flex-wrap">
      ...
      <div className='w-full md:w-9/12 rounded-md my-1'>
        ...
        <div className={`${getCardColorBG(card.rune)} w-full rounded-t p-2 flex justify-between content-center`}>
          <h1 className='text-white text-3xl text-shadow albert-sans'>{card.full_card_name || 'Art for Unknown Card'}</h1>
        ...
        <div className="my-2">
          <div className="flex mt-2">
            <div className="w-3/6">
              <div className="z-0 flex items-center ml-1 -m-px">
                <label className=" bg-content_light-500 dark:bg-content-500  text-xs font-semibold border-content2 px-1 rounded-t">CARD TYPE</label>
              </div>
            ...
            <div className="w-3/6">
              <div className="flex items-center ml-1 -m-px">
                <label className={`${getCardColorBG(card.rune)} text-xs font-semibold text-white border-content2 px-1 rounded-t`}>RUNE</label>
              </div>
              <div className={`${getCardColorBG(card.rune)} px-4 uppercase flex items-center pb-0.5 tracking-widest text-white rounded antonio`}>
          ...
          <div className="flex flex-wrap mt-2">
            ...
            {card.cost && (
              <div className="w-2/6 md:w-1/6">
                <div className="flex items-center ml-2.5 -m-px">
                  <label className="bg-content_light-500 dark:bg-content-500 text-xs font-semibold border-content3 px-1 border border-b-0 rounded-t">COST</label>
                </div>
                <div className="mx-0.5 h-8 bg-content_light-500 dark:bg-content-500 rounded border border-content3 flex items-center justify-center font-semibold text-lg ">
                  {card.cost}
                </div>
            ...
            {card.recycle.length > 0 && (
              <div className="w-2/6 md:w-1/6">
                <div className="flex items-center ml-2.5 -m-px">
                  <label className="bg-content_light-500 dark:bg-content-500 text-xs font-semibold border-content3 px-1 border border-b-0 rounded-t">CYCLE</label>
                </div>
            ...
            <div className="md:w-3/6 mt-1 md:mt-0">
              <div className="flex items-center ml-1 -m-px">
                <label className="text-xs font-semibold px-1 border">TAGS</label>
              </div>
              <div className="mx-1 h-8 flex items-center flex-wrap">
                {card.tags?.map((tag) => (
                  <div className="bg-slate-950 rounded-md mx-1 text-center text-white text-sm font-semibold px-1 py-0.5 border-slate-600 border">
                    {tag.tag}
                  </div>
                ))}
              </div>
          ...
          {card.abilities_text && (
            <div className="mt-2">
              <div className="flex items-center ml-1 -m-px">
                <label className=" bg-content_light-500 dark:bg-content-500  text-xs font-semibold border-content2 px-1 rounded-t">EFFECT</label>
              </div>
