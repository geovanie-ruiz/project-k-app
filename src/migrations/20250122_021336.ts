...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  ALTER TABLE "articles" ADD COLUMN IF NOT EXISTS "article_search_vector" "tsvector" GENERATED ALWAYS AS (to_tsvector('english', "articles"."title" || ' ' || "articles"."excerpt")) STORED;
  ALTER TABLE "cards" ADD COLUMN IF NOT EXISTS "card_name_vector" "tsvector" GENERATED ALWAYS AS (to_tsvector('english', "cards"."full_card_name")) STORED;
  ALTER TABLE "cards" ADD COLUMN IF NOT EXISTS "card_text_vector" "tsvector" GENERATED ALWAYS AS (to_tsvector('english', "cards"."abilities_text")) STORED;
  ALTER TABLE "cards" ADD COLUMN IF NOT EXISTS "card_name_text_vector" "tsvector" GENERATED ALWAYS AS (to_tsvector('english', "cards"."full_card_name" || ' ' || "cards"."abilities_text")) STORED;
  ALTER TABLE "decks" ADD COLUMN IF NOT EXISTS "likes" numeric DEFAULT 0 NOT NULL;
  ALTER TABLE "decks" ADD COLUMN IF NOT EXISTS "preview" varchar NOT NULL;
  ALTER TABLE "decks" ADD COLUMN IF NOT EXISTS "deck_name_vector" "tsvector" GENERATED ALWAYS AS (to_tsvector('english', "decks"."name")) STORED;
